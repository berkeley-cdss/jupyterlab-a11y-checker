...
const JUPYTERLITE_BASE_URL =
  "https://berkeley-cdss.github.io/jupyterlab-a11y-checker/lab/index.html";
...
function generateMarkdownReport(results: NotebookResult[]): string {
  ...
  lines.push("---");
  lines.push(
    "*Report generated by [jupycheck](https://github.com/berkeley-cdss/jupyterlab-a11y-checker)*",
  );
...
(async () => {
  try {
    const res = await fetch(
      "https://api.github.com/repos/berkeley-cdss/jupyterlab-a11y-checker",
    );
    if (res.ok) {
      const data = await res.json();
